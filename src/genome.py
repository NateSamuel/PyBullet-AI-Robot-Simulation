...
class Genome():
    ...
    def get_gene_spec():
        gene_spec =  {"link-shape":{"scale":1}, 
            "link-radius": {"scale":0.35, "default": 0.4},
            "link-length": {"scale":2, "default": 2},
            "link-recurrence": {"scale":2},
            "link-mass": {"scale":0.1, "default": 0.1},
            "joint-type": {"scale":1},
            "joint-parent":{"scale":1},
            #"joint-axis-xyz": {"scale":1},
            "joint-axis-xyz-1": {"scale": 0.0, "default": 0.0},  # x
            "joint-axis-xyz-2": {"scale": 1.0, "default": 1.0},  # y
            "joint-axis-xyz-3": {"scale": 1.0, "default": 1.0},  # z
            "joint-origin-rpy-1":{"scale":np.pi * 2},
            "joint-origin-rpy-2":{"scale":np.pi * 2},
            "joint-origin-rpy-3":{"scale":np.pi * 2},
            "joint-origin-xyz-1":{"scale":0.9},
            "joint-origin-xyz-2":{"scale":0.9},
            "joint-origin-xyz-3":{"scale":0.9},
            "control-waveform":{"scale":1},
            "control-amp":{"scale":10},
            "control-freq":{"scale":8}
            }
        ind = 0
    ...
    def get_gene_dict(gene, spec):
        ...
        for key in spec:
            ...
            default = spec[key].get("default", 0.0)
            gdict[key] = max(default + (gene[ind] * scale), 0.0001)
        return gdict
    @staticmethod
    ...
    def genome_to_links(gdicts):
        ...
        for gdict in gdicts:
            ...
            for key in ["link-length", "link-radius", "link-mass"]:
                val = gdict[key]
                if not isinstance(val, (int, float)) or val <= 0 or math.isnan(val) or math.isinf(val):
                    print(f"[Invalid Gene] {key} = {val}. Skipping malformed genome.")
                    return None  # Invalid creature
            ...
            link = URDFLink(
                ...
                joint_axis_xyz=[
                    gdict["joint-axis-xyz-1"],
                    gdict["joint-axis-xyz-2"],
                    gdict["joint-axis-xyz-3"]
                ],
        ...
        # Fix the root link so it links to nothing
...
class URDFLink:
    ...
    def to_joint_element(self, adom):
        ...
        axis = np.array(self.joint_axis_xyz)
        ...
        axis_tag.setAttribute("xyz", axis_str)
        
        limit_tag = adom.createElement("limit")
        # effort upper lower velocity
        limit_tag.setAttribute("effort", "500")
