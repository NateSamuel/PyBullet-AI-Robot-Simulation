...
def environment_creation(arena_size = 20):
    p.setGravity(0, 0, -10)
    ...
    bounding_box_min, bounding_box_max = p.getAABB(mountain)
    peak_x = (bounding_box_min[0] + bounding_box_max[0]) / 2
    peak_y = (bounding_box_min[1] + bounding_box_max[1]) / 2
    peak_z = bounding_box_max[2]
    peak_position = (peak_x, peak_y, peak_z)
    
    p.addUserDebugLine(
        peak_position,
        (peak_x, peak_y, peak_z + 1),
        [1, 0, 0], 
        lineWidth=3,
        lifeTime=0
    )
